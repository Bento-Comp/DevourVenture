...
namespace UniSkin
{
	[AddComponentMenu("UniSkin/SkinItem_FloatSequence")]
	public class SkinItem_FloatSequence : SkinItem_FloatBase
	{
		...
		public override float GetFloat(int index = 0, int count = 1)
		{
			...
			else
			{
				...
				{
					Debug.LogError("SkinItem_FloatSequence : index out of range : index = " + index + " | valuesCount = " + valuesCount + " | count = " + count);
				}
				#endif
			}
